"""Django integration for autolemetry."""
...
class AutolemetryMiddleware(MiddlewareMixin):  # type: ignore[misc]
    """
    ...
    def _ensure_initialized(self) -> None:
        """Ensure autolemetry is initialized."""
        global _INITIALIZED
        if not _INITIALIZED:
            autolemetry_config = getattr(settings, "AUTOLEMETRY", {})
            service_name = autolemetry_config.get("SERVICE_NAME", "django-app")
            endpoint = autolemetry_config.get("ENDPOINT", "http://localhost:4318")
    ...
    def process_request(self, request: Any) -> None:
        ...
        request._autolemetry_span = span
        request._autolemetry_start_time = time.time()

        return None

    def process_response(self, request: Any, response: Any) -> Any:
        """Process outgoing response - end trace span."""
        if hasattr(request, "_autolemetry_span"):
            span = request._autolemetry_span
            start_time = getattr(request, "_autolemetry_start_time", time.time())
