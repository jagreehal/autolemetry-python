...
def instrument_mcp_client(
    ...
    client.__autolemetry_mcp_client__ = True
    return client
...
def instrument_mcp_server(
    ...
    server.__autolemetry_mcp_server__ = True
    return server
...
def enable_mcp_auto_instrumentation(logger: Any | None = None) -> None:
    ...
    that new instances are wrapped with autolemetry instrumentation. This is
    ...
    try:
        ...
        if logger and hasattr(logger, "warning"):
            logger.warning(
                "autolemetry: failed to enable MCP auto-instrumentation",
                exc_info=exc,
            )
...
def _patch_mcp_class(
    ...
    for name in names:
        ...
        if getattr(cls, "__autolemetry_mcp_patched__", False):
            continue
        ...
        cls.__autolemetry_mcp_patched__ = True
...
def _wrap_client_method(
    ...
    *,
    ...
    for name in names:
        ...
        if getattr(original, "__autolemetry_mcp_wrapped__", False):
            return

        is_async = inspect.iscoroutinefunction(original)
        wrapped = _make_client_wrapper(original, operation, config, allow_string_name, is_async)
        wrapped.__autolemetry_mcp_wrapped__ = True  # type: ignore[attr-defined]
