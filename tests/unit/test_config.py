...
from autolemetry.config import AutolemetryConfig


def test_valid_config() -> None:
    """Test valid configuration."""
    config = AutolemetryConfig(service="test-service")
...
def test_invalid_endpoint() -> None:
    ...
    with pytest.raises(ValueError, match="endpoint must start with"):
        AutolemetryConfig(service="test", endpoint="invalid-endpoint")
...
def test_invalid_service() -> None:
    ...
    with pytest.raises(ValueError, match="service name cannot be empty"):
        AutolemetryConfig(service="")


def test_batch_timeout_validation() -> None:
    """Test batch timeout validation."""
    # Valid
    config = AutolemetryConfig(service="test", batch_timeout=1000)
    assert config.batch_timeout == 1000

    # Too low
    with pytest.raises(ValueError):
        AutolemetryConfig(service="test", batch_timeout=50)

    # Too high
    with pytest.raises(ValueError):
        AutolemetryConfig(service="test", batch_timeout=100000)


def test_queue_size_validation() -> None:
    """Test queue size validation."""
    # Valid
    config = AutolemetryConfig(service="test", max_queue_size=1000)
    assert config.max_queue_size == 1000

    # Too low
    with pytest.raises(ValueError):
        AutolemetryConfig(service="test", max_queue_size=0)
