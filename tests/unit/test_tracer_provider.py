...
from autolemetry.tracer_provider import (
    get_autolemetry_tracer,
    get_autolemetry_tracer_provider,
    set_autolemetry_tracer_provider,
)
...
def test_get_tracer_provider_default() -> None:
    ...
    set_autolemetry_tracer_provider(None)
...
def test_set_and_get_tracer_provider() -> None:
    ...
    # Set it
    set_autolemetry_tracer_provider(custom_provider)
...
def test_get_autolemetry_tracer_without_provider() -> None:
...
def test_get_autolemetry_tracer_with_provider() -> None:
...
def test_get_autolemetry_tracer_with_version() -> None:
    """Test getting tracer with version and schema URL."""
    provider = TracerProvider()
    set_autolemetry_tracer_provider(provider)

    tracer = get_autolemetry_tracer(
...
def test_isolated_provider_independence() -> None:
    ...
    set_autolemetry_tracer_provider(isolated_provider)

    # Get tracer from isolated provider
    isolated_tracer = get_autolemetry_tracer("isolated")
...
def test_provider_switching() -> None:
    ...
    # Use first provider
    set_autolemetry_tracer_provider(provider1)
    tracer = get_autolemetry_tracer(__name__)
    ...
    set_autolemetry_tracer_provider(provider2)
    tracer = get_autolemetry_tracer(__name__)
