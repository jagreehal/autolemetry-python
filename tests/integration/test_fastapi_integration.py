...
from autolemetry import InMemorySpanExporter, SimpleSpanProcessor, init
...
def app(exporter: Any) -> Any:
    """Create FastAPI app with autolemetry."""
    # Initialize autolemetry with test exporter
    init(service="test-api", span_processor=SimpleSpanProcessor(exporter))

    app = FastAPI()
    app.add_middleware(AutolemetryMiddleware, service="test-api")
