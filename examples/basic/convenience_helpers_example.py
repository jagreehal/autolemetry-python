...
def process_user_request(user_id: str, action: str) -> None:
    ...
    with autolemetry.span("process_request"):
        # Set single attribute (convenience helper)
        autolemetry.set_attribute("user.id", user_id)

        # Set multiple attributes at once (convenience helper)
        autolemetry.set_attributes({
            "action.type": action,
        ...
        autolemetry.add_event("request.started", {
        ...
        trace_id = autolemetry.get_trace_id()
        span_id = autolemetry.get_span_id()
...
def process_with_baggage(tenant_id: str, user_id: str) -> None:
    ...
    with autolemetry.with_baggage({
        "tenant.id": tenant_id,
        "user.id": user_id,
    }), autolemetry.span("tenant_operation"):
        # Read single baggage value (convenience helper)
        current_tenant = autolemetry.get_baggage("tenant.id")
        print(f"Processing for tenant: {current_tenant}")

        # Read all baggage (convenience helper)
        all_baggage = autolemetry.get_all_baggage()
        print(f"Full context: {all_baggage}")

        # Add more baggage if needed (convenience helper)
        autolemetry.set_baggage_value("request.id", "req-12345")

        # Nested operation inherits baggage
        with autolemetry.span("nested_operation"):
            nested_tenant = autolemetry.get_baggage("tenant.id")
...
def compare_approaches() -> None:
    ...
    with autolemetry.span("manual_example"):
    ...
    with autolemetry.span("convenience_example"):
        # Convenience: Direct helpers, no manual checks
        autolemetry.set_attributes({
            "method": "convenience",
            "status": "active",
        })
        autolemetry.add_event("processing")
