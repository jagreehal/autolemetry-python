"""
AFTER AUTOLEMETRY: Automatic OpenTelemetry instrumentation
This is what developers write WITH autolemetry.
~95% less boilerplate!
"""
...
autolemetry.init(
    ...
    subscribers=[
        ...
        autolemetry.subscribers.SlackSubscriber(webhook_url="...") if needed else None,
    ]
)
# ==================== THAT'S IT ====================
...
def ask_question() -> None:
    ...
    # No manual metric recording - autolemetry tracks HTTP metrics automatically!
    ...
    if not question:
        # Autolemetry automatically captures error state, status codes, latency
        return jsonify({"error": "Missing question"}), 400

    # For custom spans, use simple context manager
    with autolemetry.span("llm_call") as ctx:
    ...
    # No manual latency recording - autolemetry tracks it automatically!
    # No manual status code attributes - autolemetry captures them!
