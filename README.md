...
from autolemetry import init, track
from autolemetry.subscribers import PostHogSubscriber
...
**Without autolemetry** (80+ lines of setup + 25+ lines per endpoint):
...
**With autolemetry** (3 lines of setup + 0 lines per endpoint):

```python
import autolemetry

# One-line setup replaces 80+ lines!
autolemetry.init(service="my-service", instrumentation=["flask"])
...
def create_order():
    ...
    autolemetry.track("order.created", {
        "order_id": order.id,
        "total": data["total"]
    })
...
**Without autolemetry** (135 lines for MCP context propagation):
...
**With autolemetry** (0 lines - automatic):

```python
autolemetry.init(service="mcp-server", instrumentation=["mcp"])
...
async with MCPServerStdio(
    ...
    # autolemetry patches MCPServer/MCPServerStdio so handlers are traced and
...
| Pattern | Manual OTEL | Autolemetry | Reduction |
|---------|-------------|-------------|-----------|
...
```python
from autolemetry import trace_llm
...
```python
from autolemetry import trace_db
...
```python
from autolemetry import trace_http
...
```python
from autolemetry import trace_messaging
...
```python
from autolemetry import instrument
...
```python
from autolemetry import with_new_context
...
Baggage allows you to propagate custom key-value pairs across distributed traces. Baggage is automatically included in HTTP headers when using `inject_trace_context()` from `autolemetry.http`.

**Basic usage:**

```python
from autolemetry import trace, with_baggage
from autolemetry.http import inject_trace_context
...
```python
from autolemetry import init, trace, with_baggage
...
```python
from autolemetry import Metric
...
```python
from autolemetry import Event
from autolemetry.subscribers import WebhookSubscriber
...
**Bring your own logger** and autolemetry automatically instruments it to inject trace context.
...
app = FastAPI()
app.add_middleware(AutolemetryMiddleware, service="my-api")
```
...
# settings.py
MIDDLEWARE = [
    'autolemetry.integrations.django.AutolemetryMiddleware',
    # ... other middleware
]
...
from autolemetry.integrations.flask import init_autolemetry

app = Flask(__name__)
init_autolemetry(app, service="my-flask-app")
```
...
```python
from autolemetry import init, AdaptiveSampler
...
```python
from autolemetry import RateLimiter
...
```python
from autolemetry import CircuitBreaker
...
```python
from autolemetry import PIIRedactor
...
```python
from autolemetry.http import http_instrumented, trace_http_request, inject_trace_context
...
```python
from autolemetry.db import instrument_database, trace_db_query
...
from autolemetry.testing import (
...
```python
from autolemetry import shutdown, shutdown_sync
...
| Feature | Raw OpenTelemetry | autolemetry |
|---------|------------------|------------|
